...
import com.example.lumos.databinding.DialogEditOrderDetailsBinding
...
class EditOrderDialogFragment : DialogFragment() {
    private var _binding: DialogEditOrderDetailsBinding? = null
    ...
    private var currentArtists: List<Artist> = emptyList()

    var onUpdateOrderListener: ((
        orderId: Int,
        date: String,
        performanceId: Int,
        location: String,
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        _binding = DialogEditOrderDetailsBinding.inflate(LayoutInflater.from(requireContext()))
        order?.let { order ->
            binding.etDate.setText(order.date)
            binding.etLocation.setText(order.location)
            binding.etAmount.setText(order.amount.toString())
            binding.etComment.setText(order.comment)
            binding.switchCompleted.isChecked = order.completed
            ...
            if (currentPerformanceIndex >= 0) {
                binding.spinnerPerformance.setSelection(currentPerformanceIndex + 1)
            }

            updateArtistsSelection(binding.artistsContainer, order.performance, currentArtists)
